...
\large Tau Core Foundation Paper II}
...
This is the second part of a three-paper foundation sequence: Paper I freezes
the architecture and conditional completion, while Paper III analyzes record
transport and recovery after a source packet has been fixed.
\end{abstract}
\tableofcontents
\clearpage
...
\(\Pi_{\rm phys}\) names a physical selection target; it does not assert that
...
descriptor.\\
Physical selection & Why is one admissible packet occupied by Nature? & A
...
does not show that a physical parent selects \(\Sigma_s\).  If a signature map
...
\begin{figure}[t]
 ...
 \caption{Left: existence, uniqueness, identifiability and physical selection
...
still no proof of physical selection.\\
\(0<\delta\ll1\) & \(G>0\) but
...
nature-selection failure requires an upstream source law or independent
...
finite claim.  It does not prove that Nature realizes the audited source.
...
\begin{equation}
 ...
 \begin{cases}
  ...
  \text{explicit nature-selection blocker}.
 \end{cases}
...
\begin{enumerate}[label=(P\arabic*)]
 ...
 \item the terminally relevant descriptors without endpoint-dependent
 tuning.
\end{enumerate}
...
This distinction gives a finite reopening rule.  The physical-selection branch
...
This closes internal nonemptiness for the adopted regular branch, not physical
selection. An inequivalent completion with an independent base
causal-work/energy-sharing carrier has the same current narrow source reduct.
No selector using only that reduct can therefore establish that Nature
occupies the primary packet.
...
\begin{equation}
 ...
 \end{array}
\end{equation}
The Tau-specific blocker is the physical source of the third line and of
\(\Pi_{\rm phys}\).  Neither GNS uniqueness nor the dimension audit selects
that source.  Terminal fits cannot fill the gap without violating source
freeze.
...
\begin{itemize}
 ...
 source state, onto incidence, or \(\Pi_{\rm phys}\) as a uniquely
 Nature-selected packet; one enriched packet constructs them conditionally;
...
physical selection.  Complete source data determine a unique minimal cyclic
...
These are constructive finite targets, not nature-selection theorems.
The primary enriched branch conditionally realizes the finite target listed in
(P1)--(P6), but physical base--seed ownership and joint occupation remain
unselected. The current-reduct counterpair proves that this is not removable by
another representation theorem on the same narrow data.
...
\begin{enumerate}[label=(D\arabic*)]
 ...
 close the Tau-specific nature-selection arrow.
\end{enumerate}
...
identifiability does not imply physical selection, and a faithful quotient
